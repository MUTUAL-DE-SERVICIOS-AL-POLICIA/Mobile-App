...
class EvaluationService {
  ...
  Future<bool> saveEvaluation(SavedLoanEvaluation evaluation, {int? userId}) async {
    try {
  ...
  Future<bool> isDuplicate(SavedLoanEvaluation newEvaluation, {int? userId}) async {
  ...
  Future<bool> deleteEvaluation(String evaluationId, {int? userId}) async {
    try {
      ...
      await prefs.setString(key, jsonEncode(evaluations.map((e) => e.toJson()).toList()));
      return true;
    } catch (e) {
  ...
  Future<int> cleanObsoleteEvaluations(List<dynamic> currentModalities, {int? userId}) async {
    try {
      final evaluations = await getSavedEvaluations(userId: userId);
      ...
      for (final evaluation in evaluations) {
        ...
        final currentAnnualInterest = (params['annualInterest'] as num?)?.toDouble() ?? 0.0;
        final currentPeriodInterest = (params['periodInterest'] as num?)?.toDouble() ?? 0.0;
        ...
        final interestChanged =
            (evaluation.annualInterest - currentAnnualInterest).abs() > 0.01 ||
                (evaluation.periodInterest - currentPeriodInterest).abs() > 0.01;
      ...
      if (removedCount > 0) {
        final prefs = await SharedPreferences.getInstance();
        final key = userId != null ? '${_storageKey}_$userId' : _storageKey;
        await prefs.setString(key, jsonEncode(validEvaluations.map((e) => e.toJson()).toList()));
      }
  ...
  Future<SavedLoanEvaluation?> getEvaluationById(String evaluationId, {int? userId}) async {
    try {
      final evaluations = await getSavedEvaluations(userId: userId);
  ...
  static String getInterestLabel(int loanMonthTerm) {
    switch (loanMonthTerm) {
      case 1: return 'Interés Mensual';
      case 3: return 'Interés Trimestral';
      case 6: return 'Interés Semestral';
      case 12: return 'Interés Anual';
      default: return 'Interés Periódico';
    }
  }
  ...
  static String getTermType(int loanMonthTerm) {
    switch (loanMonthTerm) {
      case 1: return 'meses';
      case 3: return 'trimestres';
      case 6: return 'semestres';
      case 12: return 'años';
  ...
  static double calculateCuota(double amount, int termMonths, LoanParameters params) {
  ...
  static double calculateMontoMaximo(double liquidoParaCalificacion, LoanParameters params) {
    ...
    final cuotaMaximaPermitida = ingresoParaDebtIndex * (params.debtIndex / 100);
  ...
  static double calculateDebtIndex(double cuota, double liquidoParaCalificacion, LoanParameters params) {
  ...
  static double calculateTotalInterest(double amount, double cuota, int termMonths) {
